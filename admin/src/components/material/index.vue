    ...
    <div class="material" v-loading="pager.loading">
        <div class="material__left">
                        ...
                        <el-tree ref="treeRef" node-key="id" :data="cateLists" empty-text="''" :highlight-current="true"
                            ...
                            <template v-slot="{ data }">
                                <div class="flex flex-1 items-center min-w-0 pr-4">
                                    ...
                                    <el-dropdown v-perms="['albums:cateRename', 'albums:cateDel']" v-if="data.id > 0"
                                        :hide-on-click="false">
                                        <span class="muted m-r-10">···</span>
                                        <template #dropdown>
                                            <el-dropdown-menu>
                                                <popover-input v-perms="['albums:cateRename']"
                                                ...
                                                <div v-perms="['albums:cateDel']" @click="handleDeleteCate(data.id)">
            ...
            <div class="flex justify-center p-2 border-t border-br">
                <popover-input v-perms="['albums:cateAdd']" @confirm="handleAddCate" size="default" width="400px"
        ...
        <div class="material__center flex flex-col">
            <div class="operate-btn flex">
                <div class="flex-1 flex">
                    <upload v-if="type == 'image'" v-perms="['upload:image']" class="mr-3" :data="{ cid: cateId }"
                        :type="type" :show-progress="true" @change="refresh">
                        <el-button type="primary">本地上传</el-button>
                    </upload>
                    <upload v-if="type == 'video'" v-perms="['upload:video']" class="mr-3" :data="{ cid: cateId }"
                        :type="type" :show-progress="true" @change="refresh">
                        <el-button type="primary">本地上传</el-button>
                    </upload>
                    <upload v-if="type == 'audio'" v-perms="['upload:audio']" class="mr-3" :data="{ cid: cateId }"
                        :type="type" :show-progress="true" @change="refresh">
                        <el-button type="primary">本地上传</el-button>
                    </upload>
                    <el-button v-perms="['albums:albumDel']" v-if="mode == 'page'" :disabled="!select.length"
                        @click.stop="batchFileDelete()">
                        删除
                    </el-button>

                    <popup v-perms="['albums:albumMove']" v-if="mode == 'page'" class="ml-3" @confirm="batchFileMove"
                    ...
                    <ul class="file-list flex flex-wrap mt-4">
                        <li class="file-item-wrap" v-for="item in pager.lists" :key="item.id"
                            ...
                            <div class="operation-btns flex items-center">
                                <popover-input v-perms="['albums:albumRename']"
                    ...
                    <el-table-column label="操作" width="150" fixed="right">
                        <template #default="{ row }">
                            <div class="inline-block" v-perms="['albums:albumRename']">
                                <popover-input @confirm="handleFileRename($event, row.id)" size="default"
                                    :value="row.name" width="400px" :limit="50" show-limit teleported>
                                    <el-button type="primary" link> 重命名 </el-button>
                                </popover-input>
                            </div>
            ...
            <div class="material-center__footer flex justify-between items-center mt-2">
                <div class="flex">
                    <template v-if="mode == 'page'">
                        ...
                        <el-button v-perms="['albums:albumDel']" :disabled="!select.length" @click="batchFileDelete()">
                            删除
                        </el-button>
                        <popup v-perms="['albums:albumMove']" class="ml-3 inline" @confirm="batchFileMove"
                            :disabled="!select.length" title="移动文件">
                            <template #trigger>
                                <el-button :disabled="!select.length">移动</el-button>
                            </template>
                            ...
                            <div>
                                ...
                                <el-select v-model="moveId" placeholder="请选择">
                                    <template v-for="item in cateLists" :key="item.id">
                                        <el-option v-if="item.id !== ''" :label="item.name"
